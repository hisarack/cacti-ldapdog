...
if ($action == 'login') {
	...
	/* HiNet LDAP and Tree Permission Mapping(modify by zonghua) */
	if ( sizeof($user) > 0 && read_config_option("auth_method") == 3) {
		process_ldapdog_permissions($user,$ldap_dn);
	}
...
/* LDAPDOG MODULE : LDAP and Tree Permission Mapping(modify by zonghua) */
function process_ldapdog_permissions($user,$ldap_dn) {

	/* Update user fullname in MySQL by One-times LDAP Search */
        $ldap = array();
	$ldap['host'] = read_config_option("ldap_server");
	$ldap['port'] = read_config_option("ldap_port");
	$ldap['base'] = read_config_option("ldap_search_base");

	$ldapconn = ldap_connect($ldap['host'],$ldap['port']) or die("Could not connect to server");

	ldap_set_option($ldapconn,LDAP_OPT_PROTOCOL_VERSION,3);
	ldap_set_option($ldapconn,LDAP_OPT_REFERRALS,0);

        $specific_dn  = read_config_option("ldap_specific_dn");
        $specific_pwd = read_config_option("ldap_specific_password");
	$ldapbind = ldap_bind($ldapconn,$specific_dn,$specific_pwd);
	if(!$ldapbind){echo ldap_error($ldapconn);return;}

        $search_filter = read_config_option("ldap_search_filter");
	$ldapret = ldap_search($ldapconn,$ldap['base'],$search_filter);
        $ldapinfo = ldap_get_entries($ldapconn,$ldapret);
	ldap_close($ldapconn);

	$ldap_fullname = $ldapinfo[0]["fullname"][0];
	db_execute("UPDATE user_auth SET full_name=\"".$ldap_fullname."\" WHERE username = \"".$user["username"]."\""); 


	/* Update tree access permissions */
	$user_dep = "";	
      
        cacti_log("HINMS DEBUG: LDAP RAW DN is ".$ldap_dn);
	$ldap_dn_tokens = explode(",",$ldap_dn);
	foreach($ldap_dn_tokens as $ldap_dn_token){
		$ldap_ou_token = explode("=",$ldap_dn_token);
		$ldap_ou_key = $ldap_ou_token[0];
		$ldap_ou_val = $ldap_ou_token[1];
		if(strcmp($ldap_ou_key,"cn") ==  0){continue;}
		if(strcmp($user_dep,"") != 0){$user_dep=",".$user_dep;}
		$user_dep=$ldap_dn_token.$user_dep;
        }

	cacti_log("HINMS DEBUG: LDAP DN is ".$user_dep." USER NAME: ".$user["username"]." ".$ldap_fullname);
	db_execute("DELETE FROM user_auth_perms WHERE user_id = ".$user["id"]); 
	$tree_perms = db_fetch_assoc('SELECT hl2t.graph_tree_id FROM ldapdog_items hldap, ldapdog_item2tree_perms hl2t WHERE hldap.ou = "'.$user_dep.'" AND hldap.ldap_id = hl2t.ldap_id');

	foreach($tree_perms as $tree_perm){
		cacti_log("HINMS DEBUG: LDAP DN renaming is ".$user_dep);
		$tree_perm_id = $tree_perm["graph_tree_id"];
		$graph_items = db_fetch_assoc("SELECT * FROM graph_tree_items g, host h WHERE g.graph_tree_id = ".$tree_perm_id." AND g.host_id = h.id");
		cacti_log("HINMS DEBUG: Tree ID is ".$tree_perm_id);
		db_execute("INSERT INTO user_auth_perms (user_id,item_id,type) VALUES (".$user["id"].",".$tree_perm_id.","."2)");
		foreach($graph_items as $graph_item){
			cacti_log("HINMS DEBUG: Graph item Host ID is ".$graph_item["host_id"]);
			db_execute("INSERT INTO user_auth_perms (user_id,item_id,type) VALUES (".$user["id"].",".$graph_item["host_id"].","."3)");
		}
	}
} 
